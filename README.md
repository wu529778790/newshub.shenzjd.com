# 🔥 NewsHub - 热点内容聚合器

一个高性能的热点内容聚合器，从多个平台（微博、知乎、百度、Bilibili、小红书、豆瓣等）抓取热门数据，提供统一的浏览界面。

## ✨ 核心特性

- **多平台聚合**：支持 40+ 个数据源，覆盖主流社交、新闻、科技平台
- **拖拽排序**：自定义数据源显示顺序，保存到本地
- **图片分享**：一键生成精美分享图，支持 Canvas 自定义绘制
- **智能缓存**：内存缓存 + 自动过期，减少 API 调用
- **响应式设计**：完美适配移动端和桌面端
- **深色模式**：支持主题切换
- **RSS 订阅**：所有数据源提供 RSS 格式输出

## 🚀 快速开始

### 本地开发

```bash
pnpm install
pnpm dev
# 访问 http://localhost:3000
```

### Docker 部署

```bash
# Docker Hub
docker run -p 3000:3000 wu529778790/newshub.shenzjd.com:latest

# GitHub Container Registry
docker run -p 3000:3000 ghcr.io/wu529778790/newshub.shenzjd.com:latest
```

### Docker Compose

```yaml
version: '3.8'
services:
  newshub:
    image: wu529778790/newshub.shenzjd.com:latest
    ports:
      - "3000:3000"
    restart: unless-stopped
```

```bash
docker-compose up -d
```

## 🐳 构建与部署

### 自动构建

GitHub Actions 会在以下情况自动构建 Docker 镜像：
- 推送到 `main` 分支 → `latest` 标签
- 创建版本标签 → 版本号标签

镜像同时推送到：
- Docker Hub: `wu529778790/newshub.shenzjd.com`
- GHCR: `ghcr.io/wu529778790/newshub.shenzjd.com`

### 手动构建

```bash
# 构建镜像
docker build -t newshub .

# 运行容器
docker run -p 3000:3000 newshub
```

## 📊 数据源列表（40+）

### 🔥 热点平台
| 平台 | 子源 | 刷新频率 |
|------|------|----------|
| 微博 | 实时热搜 | 2 分钟 |
| 知乎 | 热榜 | 10 分钟 |
| 百度 | 实时榜 | 10 分钟 |
| B站 | 热搜/热门视频/排行榜 | 5 分钟 |
| 抖音 | 热门 | 10 分钟 |
| 虎扑 | 主干道 | 10 分钟 |
| 贴吧 | 热议 | 10 分钟 |
| 今日头条 | 热点 | 10 分钟 |
| 小红书 | 热门 | 10 分钟 |
| 豆瓣 | 电影/图书/音乐 | 30 分钟 |
| Twitter | 热门趋势 | 30 分钟 |
| Google Trends | 热门搜索 | 30 分钟 |

### 📰 新闻媒体
| 平台 | 刷新频率 |
|------|----------|
| 澎湃新闻 | 30 分钟 |
| 参考消息 | 30 分钟 |
| 卫星通讯社 | 30 分钟 |
| 凤凰网 | 10 分钟 |
| BBC News | 30 分钟 |

### 💰 金融财经
| 平台 | 子源 | 刷新频率 |
|------|------|----------|
| 雪球 | 热门股票 | 2 分钟 |
| 格隆汇 | 实时事件 | 2 分钟 |
| 华尔街见闻 | 热门/新闻 | 30 分钟 |
| 法布财经 | 新闻头条 | 30 分钟 |
| 金十数据 | 实时 | 5 分钟 |
| 东方财富 | 热点 | 10 分钟 |

### 💻 科技开发
| 平台 | 刷新频率 |
|------|----------|
| GitHub Trending | 30 分钟 |
| Hacker News | 30 分钟 |
| Product Hunt | 30 分钟 |
| Solidot | 60 分钟 |
| 少数派 | 30 分钟 |
| 稀土掘金 | 30 分钟 |
| IT之家 | 5 分钟 |
| 36氪 | 5 分钟 |

### 🎯 社区论坛
| 平台 | 子源 | 刷新频率 |
|------|------|----------|
| V2EX | 分享/热门 | 30 分钟 |
| 远景论坛 | Win11 | 5 分钟 |

### 🎬 娱乐生活
| 平台 | 刷新频率 |
|------|----------|
| 酷安 | 30 分钟 |
| 快手 | 10 分钟 |

## 🔧 常用命令

```bash
pnpm dev          # 开发模式（http://localhost:3000）
pnpm build        # 构建生产版本
pnpm preview      # 预览生产构建
pnpm type-check   # TypeScript 类型检查
pnpm generate     # 生成静态站点
```

## 🔍 API 端点

```bash
# 获取热点数据
GET /api/hot/{source}          # 例如: /api/hot/baidu
GET /api/hot/{source}?force=1  # 强制刷新缓存

# 生成分享图片
GET /api/image/{source}        # 生成热点列表图片

# RSS 订阅
GET /api/rss/{source}          # RSS 格式数据
GET /api/rss/{source}?limit=10 # 限制数量
```

## 🐳 Docker 构建与部署

### GitHub Actions 自动构建

项目配置了完整的 CI/CD 流程：

```yaml
# .github/workflows/docker-publish.yml
on:
  push:
    branches: [main]
    tags: ['v*']
  workflow_dispatch:
```

**构建流程：**
1. 推送代码到 `main` 分支或创建标签
2. 自动构建多平台镜像（linux/amd64）
3. 同时推送到 Docker Hub 和 GHCR
4. 使用构建缓存加速后续构建

**镜像仓库：**
- Docker Hub: `docker.io/wu529778790/newshub.shenzjd.com`
- GHCR: `ghcr.io/wu529778790/newshub.shenzjd.com`

### Dockerfile 特点

- **单阶段构建**：简化构建过程，减少镜像体积
- **静态文件服务**：内置 `start.mjs` 处理静态资源
- **端口 3000**：默认监听 3000 端口

### 部署示例

```bash
# 快速启动
docker run -d -p 3000:3000 \
  --name newshub \
  --restart unless-stopped \
  wu529778790/newshub.shenzjd.com:latest

# 使用环境变量
docker run -d -p 3000:3000 \
  -e CACHE_DURATION=600000 \
  -e HOST=0.0.0.0 \
  wu529778790/newshub.shenzjd.com:latest

# Docker Compose
docker-compose up -d
```

## 🛠️ 开发指南

### 添加新数据源

1. 在 `server/sources/` 创建文件：
```typescript
// server/sources/new-platform.ts
export default defineSource({
  'new-platform': async () => {
    // 实现抓取逻辑
    return [{
      id: 'unique-id',
      title: '标题',
      url: 'https://example.com',
      extra: { info: '附加信息' }
    }];
  }
});
```

2. 在 `server/services/sources.ts` 添加配置：
```typescript
{
  id: 'new-platform',
  name: '新平台',
  home: 'https://example.com'
}
```

3. 在 `server/services/hot-list.service.ts` 注册：
```typescript
import getNewPlatformList from '~/server/sources/new-platform';

const fetcherMap = {
  'new-platform': getNewPlatformList['new-platform']!,
  // ...
};
```

### 调试工具

```bash
# 查看原始数据
curl http://localhost:3000/api/hot/baidu

# 生成图片
curl http://localhost:3000/api/image/baidu -o test.png

# 强制刷新缓存
curl http://localhost:3000/api/hot/baidu?force=1

# RSS 格式
curl http://localhost:3000/api/rss/baidu
```

## 📝 环境变量

```bash
# .env (可选)
CACHE_DURATION=600000    # 缓存时间（毫秒），默认 10 分钟
MAX_RETRIES=3            # 请求重试次数，默认 3
HOST=0.0.0.0             # 服务地址，默认 0.0.0.0
PORT=3000                # 服务端口，默认 3000
```

## 🎯 核心架构

```
┌─────────────────────────────────────────┐
│           前端 (Nuxt 4 + Vue 3)          │
│  - 拖拽排序 (localStorage)              │
│  - 懒加载 (IntersectionObserver)        │
│  - 图片生成 (Canvas/HTML2Canvas)        │
└─────────────────┬───────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────┐
│        后端 API (Nitro Server)           │
│  - /api/hot/{source}                    │
│  - /api/image/{source}                  │
│  - /api/rss/{source}                    │
└─────────────────┬───────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────┐
│      数据抓取层 (server/sources/)        │
│  - myFetch() 带缓存和重试               │
│  - HTML 解析 (cheerio)                  │
│  - RSS/JSON 处理                        │
└─────────────────┬───────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────┐
│         缓存系统 (内存/持久化)           │
│  - 自动过期                             │
│  - 节流控制                             │
└─────────────────────────────────────────┘
```

## 🚀 性能优化

1. **并行请求**：所有数据源同时抓取
2. **智能缓存**：避免重复请求，减少源站压力
3. **懒加载**：图片按需加载
4. **服务端渲染**：首屏快速展示
5. **错误隔离**：单源失败不影响其他源

## 📄 许可证

MIT
